...
int main()
{
    int y=1,i=635,j=0,v=470;
    //cin>>y;
    if (y==1)
    {
    RenderWindow window(VideoMode(640,480), "Bouncing mushroom.");
    RectangleShape rec(Vector2f(5,50));
    ...
    while(window.isOpen())
    {
        ...
        while(window.pollEvent(event))
        {
            if(event.type == Event::Closed)
            {
             ...
             if (Keyboard::isKeyPressed (Keyboard::Up) && j>=0)
             {
                j-=10;
                v-=10;
             }
            if (Keyboard::isKeyPressed (Keyboard::Down) && j<=480)
            {
                j+=10;
                v+=10;
            }
        }
        if((mushroom.getPosition().x + (size.x / 2) >= v && increment.x > 0) ||(mushroom.getPosition().x - (size.x / 2) < 0 && increment.x < 0))
        {
...
int main()
{
    ...
    while (window.isOpen())
    {
        ...
        {
            player1.move(0,-moveDistance/7.0);
        }else
